...
def main():
    ...
    metrics = ["rmse", "mae","mi_acc","mi_acc_w1","mi_acc_w2", "doa", "pc-er", "rm"]

    # regex for metrics after training  ← updated to include mi_acc, mi_acc_w1, mi_acc_w2
    metric_re = re.compile(r".*?\b(rmse|mae|mi_acc(?:_w[12])?|doa|pc-er|rm)\b\s*:\s*([0-9eE\.\-]+)")

    # extract testid from filename: launch_IMPACT_Portrait_1256230_0.out
    file_re = re.compile(rf"launch_IMPACT_{ds}_([A-Za-z0-9\-]+)_\d+\.out")
